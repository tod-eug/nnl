...
public class DividendController {
    ...
    public static void calculateDivs(ArrayList<Dividend> list) {

        Map<String, Map<Date, ExchangeRate>> exchangeRates = new HashMap<>();
        exchangeRates = getExchangeRates(list);

        //filter only real received dividends
        List<Dividend> filteredList = list.stream()
                .filter(dividend -> dividend.getCode().equals("Re"))
                .filter(dividend -> dividend.getDate().equals(dividend.getPaymentDate()))
                .collect(Collectors.toList());

        ArrayList<Double> taxes = new ArrayList<>();

        //calculate
        for (Dividend d : filteredList) {
            ...
            System.out.println(dividendRub);
        }

    }


    private static Map<String, Map<Date, ExchangeRate>> getExchangeRates(ArrayList<Dividend> list) {
        Map<String, Map<Date, ExchangeRate>> exchangeRates = new HashMap<>();
    ...
    private static Map<String, Date> getDatesRange(ArrayList<Dividend> list) {
    ...
    private static List<String> getListOfCurrencies(ArrayList<Dividend> list) {
